	...
	void OnGUI()
	{
		...
		if (micConnected)
		{
			...
			if (!Microphone.IsRecording(null))
			{
				...
				if (GUI.Button(new Rect(Screen.height / 4 - 25, Screen.width / 4 - 100, 500, 150), "Record"))
				{
			...
			else //Recording is in progress
			{
				...
				if (GUI.Button(new Rect(Screen.height / 4 - 25, Screen.width / 4 - 100, 500, 150), "Stop and Play!"))
				{
				...
				GUI.Label(new Rect(Screen.height / 4 + 25, Screen.width / 4 - 100, 500, 150), "Recording in progress...");
			}
		}
		else // No microphone
		{
	...
	IEnumerator StartVoiceRequest(String url, String AccessToken, object parameter)
	{
		...
		if (samples != null)
		{
			...
			else
			{
				...
				responseintent = content.queryResult.intent.displayName;
				Debug.Log("Return: " + responseintent);
				// Debug.Log(content.outputAudio);
				// Debug.Log(audioSource);
                audioSource.clip = WavUtility.ToAudioClip(Convert.FromBase64String(content.outputAudio), 0);
				audioSource.Play();
